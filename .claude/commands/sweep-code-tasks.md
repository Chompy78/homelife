...
The unattended version of `/pick-code-task` → `/run-code-task`, run in a loop over eligible tasks instead of one at
...
Track a **consecutive-failure counter**, starting at 0. A "failure" is: `/run-code-task` dropping a task as
...
For each candidate:
1. **Invoke `/run-code-task <task title>`** through Step 3 (verify) — but **do not let it push yet**; stop it
...
If executing or reviewing a task surfaces genuinely new, separate work, format it in `/add-code-task`'s exact
