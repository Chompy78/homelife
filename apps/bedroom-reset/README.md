...
Up to 3 reference photos showing what a properly tidy room looks like are
shown right above the checklist - a visual target to aim for. Tap one to
view it full-size. Only a parent can add or remove these, from the parent
dashboard - a kid's app is view-only here, so they can't clear their own
target photos. Photos are stored privately in Supabase Storage - the
bucket isn't public, and every photo is served through a short-lived
signed URL minted by the edge function, never a permanent public link.
