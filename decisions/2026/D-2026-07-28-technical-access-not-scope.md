...
confirmed a real gap: a session with broad, non-enforced filesystem/connector access (there, a Windows app's
drive mapping covering multiple projects, not a project-scoped one) reasoned it *would* edit a different
project's files if asked, since it saw no rule stopping it.
...
**Status:** Done. See AI_templates' `D-2026-07-28-technical-access-not-scope` for the full reasoning.
