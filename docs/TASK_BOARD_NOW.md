...
4. **`llava:13b` room-validity gate** - perception only. The model
   ...
   `temperature: 0`.
5. **Room fingerprint** (generated on-demand, cached) - **correction,
   2026-08-02:** confirmed against the user's actual live `poller.py`
   that this is *not* wired into the scoring pipeline below, despite
   this section previously describing it that way. Per that file's own
   docstring, it's a distinct poll (`get_pending_fingerprint_regenerations`)
   triggered only when a parent asks for a fresh description from the
   dashboard - purely a parent-facing text field
   ...
   `delete_family_room_photo`), generated by one `llava:13b` call over
   the reference photos and stored via `submit_room_fingerprint`. This
   matches the drift `D-2026-07-17-poller-fingerprint-generation`
   already flagged (fingerprint added as additive, never wired into
   `process_job`/`llava_score`) - this task's design notes just hadn't
   been corrected to say so until now.
   **2026-08-02 update:** the fingerprint text itself was still leaning
   ...
   backstop, applied directly to the user's `poller.py` and delivered
   back to them - not yet confirmed live.
6. **`llava:13b` scorer** - only reached if all above pass. Compares
   the submission directly against the raw reference photos for *both*
   room-identity ("same bed, same flooring, same major furniture") and
   tidiness scoring (1-10 with explicit ranges, one encouraging
   sentence + exactly 3 specific actions) in one call - it does **not**
   read the fingerprint text (see the correction under step 5). This
   means ordinary bedding differences can still, in principle, trip the
   room-match step the way `D-2026-07-16-room-fingerprint` originally
   set out to fix - that original fix was never actually connected to
   scoring. Whether to wire the fingerprint into this step for real is
   an open question, not yet a task.
...
solid - direct measurements, can't hallucinate. The scorer's room-match
step still compares raw photos directly (see the 2026-08-02 correction
under step 5/6 above) - the bedding false-rejection
`D-2026-07-16-room-fingerprint` set out to fix is not actually
resolved by the fingerprint system as currently wired, since scoring
never reads it. The AI layers overall are still vision-model judgment
underneath software rules, not proven infallible - see the
